...
GamepadManager.Gamepad gamepad;

gamepad = GamepadManager.activeManager.GetGamepadAtSlot(0);			// Gampad at specific slot
gamepad = GamepadManager.activeManager.GetCombinedGamepad();		// Combined state of all active gamepads

if (gamepad != null)
	{
	gamepad.GetKey(GamepadManager.GamepadKey.L1);
	gamepad.GetKeyAnalog(GamepadManager.GamepadKey.L1);
	gamepad.leftStick.state.GetVector();
